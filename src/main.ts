...
type ChapterId = 'home' | '00-matter' | '01-electrostatics' | '02-circuits' | '03-magnetism' | '04-induction' | '05-maxwell' | '06-geo-optics' | '07-wave-optics' | 'design' | 'lab';
...
function updateMarginRail(id: ChapterId) {
  ...
  // Prepend bookmark card for real chapters (not home, design, or lab)
  const meta = CHAPTERS.find(c => c.id === id);
  const bookmarkCardHtml = meta && id !== 'home' && id !== 'design' && id !== 'lab'
